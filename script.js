(function() {
    ...
    function load() {
        ...
        if (stored) {
            ...
            // Initialize with empty array - no seed data
            expenses = [];
        }
    }
    
    function save() { 
        localStorage.setItem('zenith_expenses', JSON.stringify(expenses)); 
    }
    ...
    function updateChart() {
        ...
        const data = cats.map(c => 
            expenses.filter(e => e.category === c).reduce((a, ex) => a + (Number(ex.amount) || 0), 0)
        );
    ...
    // --- init ---
    dateInp.value = new Date().toISOString().split('T')[0];
    load(); // Load from localStorage (will be empty array initially)
    save(); // Save empty array to localStorage
    refresh(); // Refresh UI with zero data
